...
- `as: symbol | string` on `forRoot()` names a registration. Registering one token twice throws; selecting an unconfigured token fails at startup; `as` with `isGlobal: true` throws and `as` with `isGlobal: false` is inert — a named registration is already never `@Global()`, and neither spelling changes `DrizzleModule`'s globality

...
`,
  ...
  describe('Error handling', () => {
    ...
    it('should throw error when accessing uninitialized config', () => {
      ...
      expect(() => config.get('test')).toThrow('Configuration not initialized');
    });
    ...
    it('should throw error when getting values before initialization', () => {
      ...
      expect(() => config.getSafeConfig()).toThrow('Configuration not initialized');
    });
  });
        ...
        security: {
          ...
          apiSecret: Env.string({
            ...
            validate(secret: string) {
              if (secret.length < 10) {
                return Effect.fail(
                  new EnvValidationError('', secret, 'API secret must be at least 10 characters'),
                );
              }
